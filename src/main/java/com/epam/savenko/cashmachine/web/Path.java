package com.epam.savenko.cashmachine.web;

public class Path {
    private Path() {
    }
    public static final String PAGE_LOGIN="index.jsp";
    public static final String PAGE_BAD_LOGIN="/badpage.jsp";
    public static final String PAGE_ADD_PRODUCT="/add_product.jsp";
    public static final String PAGE_PRODUCTS_LIST="/WEB-INF/jsp/products_list.jsp";
    public static final String PAGE_ORDERS_LIST="/WEB-INF/jsp/orders_list.jsp";
    public static final String PAGE_TO_ADD_PRODUCTS="/WEB-INF/jsp/add_product.jsp";
    public static final String PAGE_REGISTRATION="register.jsp";
    public static final String PAGE_MAIN="/WEB-INF/jsp/main.jsp";
    public static final String PAGE_NO_COMMAND="/hello-servlet";
}
